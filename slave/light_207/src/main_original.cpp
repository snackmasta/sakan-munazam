...
LightController lightController(LIGHT_PIN, &ldrSensor);
CommandHandler commandHandler(&lightController, &ldrSensor, &udpHandler, DEVICE_ID);
StatusReporter statusReporter(&lightController, &ldrSensor, &udpHandler, DEVICE_ID);

// Timing variables
unsigned long lastUDPBroadcast = 0;
unsigned long lastLightAdjust = 0;
const long udpBroadcastInterval = 500;
const long lightAdjustInterval = 100;
...
void handleUDPMessage(String message) {
    ...
    if (message == "ON") {
        ...
        currentPWM = 0;
        // Send OFF status to master
        String statusMsg = String(DEVICE_ID) + ":OFF";
        IPAddress masterIP(192,168,137,1);
        uint16_t masterPort = 4210;
